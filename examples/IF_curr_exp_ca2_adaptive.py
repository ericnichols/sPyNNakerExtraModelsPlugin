...
# Journal of Computational Neuroscience, 10(1), 25-45. doi:10.1023/A:1008916026143
#-----------------------------------------------------------------------------
import math
import numpy
import pylab
...
# Timestep (ms)
# **NOTE** the 2.5Khz input frequency is not going to work particularily well at 1ms
dt = 0.1
...
isi_cv = {t: math.sqrt(
    numpy.sum(numpy.power(time_binned_isis[t] - mean_isi, 2)) / float(len(time_binned_isis[t]))
    ) / mean_isi
...
axes[0].scatter(list(mean_isis.iterkeys()), [1000.0 / i for i in mean_isis.itervalues()], s=2)
