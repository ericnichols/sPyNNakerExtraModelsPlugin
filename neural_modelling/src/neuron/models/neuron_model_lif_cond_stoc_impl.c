...
bool neuron_model_state_update(input_t exc_input, input_t inh_input,
    ...
    if (neuron->refract_timer < 1) {
        ...
        if (ishigh) {
            neuron->V_membrane = neuron->V_rev_E - 0.5k;
        }
        bool islow = REAL_COMPARE(neuron->V_membrane, <=, neuron->V_rev_I);
        if (islow) {
            neuron->V_membrane = neuron->V_rev_I + 0.5k;
        }
        ...
        if (exponent < 5.0k) {
            ...
            result = (1. - expk(-hazard *
                                global_params->machine_time_step_ms_div_10)) *
                      PROB_SATURATION;
...
neuron_pointer_t neuron_model_lif_cond_impl_create(REAL V_thresh, REAL V_reset,
        REAL V_rest, REAL V_rev_E, REAL V_rev_I, REAL R,
