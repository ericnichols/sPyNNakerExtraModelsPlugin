#include "neuron_model_lif_cond_stoc_impl.h"
...
bool neuron_model_state_update(input_t exc_input, input_t inh_input,
    ...
    if (neuron->refract_timer < 1) {
        ...
        _lif_neuron_closed_form(neuron, V_last, -neuron->refract_timer,
                                input_this_timestep);
        bool ishigh = REAL_COMPARE(neuron->V_membrane, >=, neuron->V_rev_E);
        if (ishigh) {
            neuron->V_membrane = neuron->V_rev_E - 0.5;
        }
        bool islow = REAL_COMPARE(neuron->V_membrane, <=, neuron->V_rev_I);
        if (islow) {
        ...
        REAL exponent = (neuron->V_membrane - neuron->theta)
                        * neuron->du_th_inv;
        ...
        UREAL result;
        if (exponent < 5.0) {
            REAL hazard = expk(exponent) * neuron->tau_th_inv;
            result = (1. - expk(-hazard * refractory_time_update))
                     * PROB_SATURATION;

        } else {
            result = PROB_SATURATION;
        }
        log_debug("%1.6R", result);

        // has it spiked?
		spike = REAL_COMPARE(result, >=, random_number);
...
void neuron_model_print(restrict neuron_pointer_t neuron) {
    ...
    log_debug( "V thresh (theta) = %11.4k mv", neuron->theta );
    ...
    log_debug( "inverse threshold du  = %11.4k mv", neuron->du_th_inv );
    log_debug( "inverse threshold tau = %11.4k mv", neuron->tau_th_inv);
...
neuron_pointer_t neuron_model_lif_cond_impl_create(REAL V_thresh, REAL V_reset,
        REAL V_rest, REAL V_rev_E, REAL V_rev_I, REAL du_th_inv,
        REAL tau_th_inv, REAL theta, REAL one_over_tauRC, REAL R,
